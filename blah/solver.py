...
for it in range(niter): 
    ...
    #data = data[:, :, ::-1]
    cv2.imwrite("data.jpg", data * 255.0,[cv2.IMWRITE_JPEG_QUALITY, 100])

    label = solver.net.blobs['label'].data[0]
    label = clear.transpose((1, 2, 0));
    #clear = clear[:, :, ::-1]
    cv2.imwrite("label.jpg", label * 255.0,[cv2.IMWRITE_JPEG_QUALITY, 100])

    out = solver.net.blobs['sum'].data[0]
    out = out.transpose((1, 2, 0));
    #out = out[:, :, ::-1]
    cv2.imwrite("out.jpg", out * 255.0,[cv2.IMWRITE_JPEG_QUALITY, 100])
